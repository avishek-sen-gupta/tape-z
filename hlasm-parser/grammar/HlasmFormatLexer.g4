lexer grammar HlasmFormatLexer;


// =========================================================
// WHITESPACE AND COMMENTS (skipped during parsing)
// =========================================================

// Whitespace: spaces, tabs, carriage returns, and line feeds
WHITESPACE
    : [ \t\r\n]+ -> skip
    ;

// Single-line comment: starts with // and continues until end of line
LINE_COMMENT
    : '//' ~[\r\n]* -> skip
    ;

// Multi-line comment: enclosed between /* and */
BLOCK_COMMENT
    : '/*' .*? '*/' -> skip
    ;

fragment A:('a'|'A');
fragment B:('b'|'B');
fragment C:('c'|'C');
fragment D:('d'|'D');
fragment E:('e'|'E');
fragment F:('f'|'F');
fragment G:('g'|'G');
fragment H:('h'|'H');
fragment I:('i'|'I');
fragment J:('j'|'J');
fragment K:('k'|'K');
fragment L:('l'|'L');
fragment M:('m'|'M');
fragment N:('n'|'N');
fragment O:('o'|'O');
fragment P:('p'|'P');
fragment Q:('q'|'Q');
fragment R:('r'|'R');
fragment S:('s'|'S');
fragment T:('t'|'T');
fragment U:('u'|'U');
fragment V:('v'|'V');
fragment W:('w'|'W');
fragment X:('x'|'X');
fragment Y:('y'|'Y');
fragment Z:('z'|'Z');
fragment ZERO: '0';
fragment ONE: '1';
fragment TWO: '2';
fragment PLUS: '+';
OPTIONAL_OPEN_PAREN: '[';
OPTIONAL_CLOSE_PAREN: ']';
OPEN_PAREN: '(';
CLOSE_PAREN: ')';

DISPLACEMENT_PREFIX: D;

FLOATING_POINT_REGISTER_PAIR: F F;
FLOATING_POINT_REGISTER: F;
INDEX_REGISTER: X;
BASE_REGISTER: B;
CONTROL_REGISTER: C R;
ACCESS_REGISTER: A R;
REGISTER_PAIR: R R;
REGISTER_OPERAND: R;
IMMEDIATE_VALUE_PREFIX: I;
SIGNED_IMMEDIATE_VALUE: PLUS I;
LENGTH_FIELD_PREFIX: L;
MASK_FIELD_PREFIX: M;
RELATIVE_IMMEDIATE_OPERAND_PREFIX: R I;
VECTOR_REGISTER_PAIR: V V;
VECTOR_REGISTER: V;
COMMA: ',';

// =========================================================
// IDENTIFIERS AND LITERALS
// =========================================================

// Identifier: starts with letter or underscore, followed by letters, digits, or underscores
//IDENTIFIER
//    : [a-zA-Z_][a-zA-Z_0-9]*
//    ;

// Integer literal: sequence of digits
INTEGER_LITERAL
    : [0-9]+
    ;

// String literal: text enclosed in double quotes, with proper escape handling
STRING_LITERAL
    : '"' ( ~["\r\n\\] | '\\' . )* '"'
    ;

